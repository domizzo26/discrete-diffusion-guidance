...
def encode_image_for_reconstruction(
    ...
    if mask_type == 'partial':
        ...
        mask_map = torch.zeros((height, width), dtype=torch.bool)
        attempts = 0
        # Keep adding blocks until we hit the target area or max attempts
        while mask_map.sum() < target_area and attempts < 2000:
            y = torch.randint(0, height - block_size + 1, (1,)).item()
            x = torch.randint(0, width - block_size + 1, (1,)).item()
            
            # Only apply if the block does not overlap with existing masks
            if not mask_map[y:y+block_size, x:x+block_size].any():
                mask_map[y:y+block_size, x:x+block_size] = True
            attempts += 1
